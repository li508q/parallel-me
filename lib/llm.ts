// LLM 适配 — 真 LLM 优先；无 key 走"演员模式"，仍能完整演出
import { SELVES, NOWME, type SelfId } from "./selves";
...
export async function chat(messages: Msg[], opts?: { temperature?: number; max_tokens?: number }): Promise<string> {
...
const PLAYBOOK: Record<SelfId, Record<Topic, string[]>> = {
  ...
  money: {
    ...
    relationship: [
      "算笔账。\n谈恋爱这件事年均成本 ¥30k–¥80k 不等，结婚一次性支出 ¥150k 起，离婚成本 ¥200k+。\n这不是劝你别爱，是告诉你：每一次“再忍忍”都有具体价签。\n现金流为王。"
    ],
    family: [
  ...
  roam: {
    ...
    relationship: [
      "你跟这个人之所以僵在这里，是因为你们用一样的房间、一样的对话、一样的周末试图变出新的感情。\n换地方。哪怕只是搬到城市的另一边。\n关系只在新空间里能再生。"
    ],
    family: [
      "你妈不是控制你，是因为你一直在她够得着的范围里。\n离 1500 公里以外，你们就会开始用尊重的方式说话。\n机票就是治愈剂。"
    ],
    money: [
      "钱不是通过守住挣的，是通过站对位置挣的。\n同样的能力在不同城市差 3 倍。\n地理套利，是普通人最后的杠杆。"
    ],
    lifestyle: [
      "想象一下：早上 7 点海边醒来，光从落地窗洒进来，没有微信红点。\n你以为这是奢侈，其实在大理租这种房子 ¥3500/月。\n你不是没钱，你是没敢。"
    ],
    general: [
      "你之所以痛苦，不是问题难，是你站的位置不对。\n换地方比改自己快十倍。\n机票还没买？打开浏览器。"
    ]
...
「下一步」
今晚睡前给我妈打个 7 分钟电话——不谈这件事，只问她吃了什么。
剩下的，明天再想。`,
  relationship: `「我听到了什么」
...
「下一步」
今晚单独给妈发个语音 30 秒，不解释、不辩论，就讲一件你今天的小事。`,
  money: `「我听到了什么」
...
「下一步」
今晚做一张 Excel：列出未来 12 个月每月的现金流。看清楚，再决定。`,
  lifestyle: `「我听到了什么」
...
function mockChat(messages: Msg[]): string {
  ...
  // cross-examine 模式：从 user 消息里抓 target 的名字（system prompt 里第一个「」是 target）
  if (sys.includes("cross-examine") || sys.includes("反问")) {
    // user msg pattern: "X 刚说：「..."
    const m = usr.match(/(\S+?)刚说：/);
    const target = m?.[1] || "对方";
    const crosses = [
  ...
  // 此刻的我 优先判断（system prompt 标识独特）
  if (sys.includes("此刻的我") && sys.includes("最终裁决")) {
    return NOW_PLAYBOOK[topic];
  }

  // 五个分身 — 用 selves.ts 里独有的 system prompt 关键字精确匹配
  // future 的 prompt 里有"5 年后的我"，所以放最前面避免被误吞
  if (sys.includes("已经从终点回看") || sys.includes("5 年后的版本")) return pick(PLAYBOOK.future[topic]);
  if (sys.includes("永远在踩刹车")) return pick(PLAYBOOK.lay[topic]);
  if (sys.includes("把一切换算成现金流和 ROI")) return pick(PLAYBOOK.money[topic]);
  if (sys.includes("永远在敲门、喊"))         return pick(PLAYBOOK.roam[topic]);
  if (sys.includes("妈妈视角") || sys.includes("讨妈"))  return pick(PLAYBOOK.filial[topic]);

  return `（演员模式）听到了你说的：“${usr.slice(0, 30)}…”`;
}

// ─── 公共调用 ───
export async function callSelf(selfId: SelfId | "now", userInput: string, otherSays?: string): Promise<string> {
  const me = selfId === "now" ? NOWME : SELVES[selfId];
  const messages: Msg[] = [{ role: "system", content: me.system_prompt }];
  if (otherSays) {
    messages.push({ role: "user", content: `我此刻面对的纠结：\n${userInput}\n\n刚刚另外几个我说了这些：\n${otherSays}\n\n现在轮到你说。` });
  } else {
...
export async function crossExamine(challenger: SelfId, target: SelfId, userInput: string, targetSaid: string): Promise<string> {
  const me = SELVES[challenger];
  const t = SELVES[target];
  const messages: Msg[] = [
    { role: "system", content: me.system_prompt + `\n\n现在做一件特别的事：cross-examine，反问「${t.name}」。用一句最狠也最真诚的反问，戳穿他没说出口的那部分。\n不要超过 40 字。` },
    { role: "user", content: `用户的纠结：${userInput}\n\n${t.name}刚说：「${targetSaid.slice(0,200)}」\n\n你的一句话反问：` }
  ];
  return chat(messages, { temperature: 0.95, max_tokens: 120 });
}

// 用户向某个分身追问
export async function followUp(selfId: SelfId, userInput: string, prevAnswer: string, question: string): Promise<string> {
  const me = SELVES[selfId];
  const messages: Msg[] = [
    { role: "system", content: me.system_prompt + "\n\n继续你刚才那个分身的人格、语气和价值观，回答用户对你的追问。" },
...
// 心理学解读 — 给最终页用
export async function psychInsight(userInput: string, topResponses: string[]): Promise<string> {
  const sys = `你是一位资深心理咨询师，擅长 IFS（内部家庭系统）、CBT、Schema Therapy。
现在用户面对一个纠结，他内心 5 个不同声音都说了话。
请你用 60-90 字，温柔、不评判、不打鸡血地，告诉用户：
- 他此刻最响的那个声音背后藏着什么"未被照顾的部分"
- 这个部分通常意味着什么需要
- 不要给建议，只是命名

不要用任何专业术语黑话，像朋友在咖啡馆里跟他讲话。`;
  const messages: Msg[] = [
    { role: "system", content: sys },
    { role: "user", content: `用户纠结：${userInput}\n\n他内心 5 个声音说的话:\n${topResponses.join("\n\n")}` }
  ];
  if (!API_KEY) {
    // mock psych insight
    const t = classifyTopic(userInput);
    const map: Record<Topic, string> = {
      career: "你以为你在选工作，其实你在问：我可以不优秀地被爱吗？这是一个 25 岁前后特别响的问题。它不需要答案，它需要被听见。",
      relationship: "你不是在纠结这段关系，你在试图弄清楚一件更深的事：我可以被一个人真的看见吗？这件事比恋爱本身更重要。",
      family: "你跟父母这场拉扯里，藏着一个更年幼的你——那个曾经为了让妈妈高兴而放弃自己的孩子。他不需要你赢，只需要你看见他还在。",
      money: "钱不是钱，钱是「我配」的代名词。你纠结的不是数字，是你能不能允许自己过得好一点。这个许可，得自己给自己。",
      lifestyle: "你身体在喊的，比你以为的更重要。这不是懒，是一个被压抑很久的版本的你，在用唯一会的方式抗议。听一下它。",
      general: "你不是缺答案，你缺一个允许自己不知道的瞬间。允许自己暂停，比逼自己想清楚更难，也更必要。"
    };
    return map[t];
  }
  return chat(messages, { temperature: 0.7, max_tokens: 220 });
}
