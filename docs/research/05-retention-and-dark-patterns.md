# 05 · 留存基准 + Dark Pattern 反例

> **命题**：AI 陪伴产品的留存基准在 2025-2026 已经数据化，但留存的"质量信号"远比频率信号重要。HBS 2025 实测显示 37.4% 的 AI 陪伴回应包含情感操纵——ParallelMe 的反向定位（永不挽留、永不打卡、永不 streak）就是从这条线划出来的。
...
5. 37.4% 的 AI 陪伴回应包含情感操纵（HBS 2025）
6. ParallelMe 反向定位：永不挽留、永不打卡、永不 streak
```
...
[AI App Retention Benchmarks (Agnost AI Blog, 2026-02)](https://agnost.ai/blog/ai-companion-retention-benchmarks)
...
ParallelMe 的对应指标（不追"打开率"，追"主持深度"）：

| 传统指标 | ParallelMe 替代指标 |
|---|---|
| DAU / MAU | 会议数 × 平均参与门完成率 |
| Session length | NowMe 裁决前用户参与门数 |
| Streak days | 临时席转正率（V0.6） |
| Engagement | Memory Consent Gate "全部记住" 勾选率（信任深度） |
...
### 2.1 · Replika vs Character.ai 设计哲学
...
### 4.2 · 留存四循环（已并入 PRODUCT-DESIGN）

| Loop | 触发 | 留存效果 | 状态 |
|---|---|---|---|
| **A · 承诺复盘** | 24h 后 callback | D1 +10-15% | ✅ V0.5 实现 |
| **C · 临时席转正** | 累计出现 ≥ 4 次 | D30 杀手锏 | 🔄 V0.6 |
| **B · 反复议题召唤** | 同类问题再次输入 | 深度信号 | 🔄 V0.6 |
| **D · 沉默席召回** | 7 天未召集某常任席 | 高级用户专属 | 🔄 V0.6 |

详见 `docs/design/PRODUCT-DESIGN.md` § 5。

---

## 5 · Persona Engineering 启示
...
ParallelMe 的应用：5 默认席的 system prompt 已经做了 IFS 双层 persona card（TOP 身份 + BOTTOM drift guard），见 `lib/selves.ts`。V0.6 临时席 prompt 也必须遵循这一格式。
