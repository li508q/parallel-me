...
> **命题**：ParallelMe 技术栈的关键决策依据。本文是 `docs/design/TECH-ARCHITECTURE.md` 的引用支撑档案。
> 涉及：Vercel AI SDK 5/6 / Anthropic Agent SDK / Prompt Caching / IndexedDB + Dexie / Design Tokens。
...
> "The AI SDK RSC package — which let you stream React Server Components directly from LLM responses — is still marked experimental, and **development is officially paused**."
>
...
ParallelMe 应用：SSE event schema 直接对应 typed data parts；不使用 RSC streaming（已 paused）。
...
✗ 不主用 Anthropic Agent SDK：
  - V0.5 需要 multi-provider（DeepSeek / OpenAI / Anthropic / Ollama），不能强绑定 Anthropic
...
4. **Layer cache prefixes** in multi-tenant systems
...
详见 `docs/design/TECH-ARCHITECTURE.md` § 5.5。要点：
- V0.5 暂不启用（DeepSeek 不支持，OpenAI 自动）
- V0.6+ 仅在 Anthropic provider 时启用
- 5 默认席 system prompt（~6000 字符）→ 5min TTL
- Cabinet brief（用户阁状态）→ 1h TTL
- 临时席、用户输入 → 不缓存
- LlmRuntime 抽象层预留 caching 字段
...
2. Need complex queries?                  YES → Dexie
3. Migrating existing IDB code?           YES → idb
...
- **V0.5：Dexie 4.4 + dexie-react-hooks**
- **V0.6+：可选 Dexie Cloud 或自管 Supabase 同步**

完整 schema 见 `docs/design/TECH-ARCHITECTURE.md` § 6。
...
> "The pattern that scales is three tiers:
> - **Tier 1: Primitive tokens** (raw values)
> - **Tier 2: Semantic tokens** (aliases, intent-named)
> - **Tier 3: Component tokens** (scoped)"
>
> "**For most teams, two tiers are enough.** The third tier adds governance overhead."

ParallelMe 决策：**两层即可**（primitive + semantic）。单 codebase，不需要引入 component token 层。

### 5.2 · Tailwind v4 vs v3 战术回退
...
V0.5 实测：**Next.js 14 production build 接受 Tailwind v4**，但 dev mode 的 wellknown-errors-plugin 仍按 v3 syntax 检测——拒绝启动。

V0.5 决策：**Tailwind v3.4 + tailwind.config.ts theme.extend + CSS variables 双轨**。完整 token 系统经由 `var(--...)` 引用 CSS variables 完整保留。

### 5.3 · 多源交叉
- [Bootspring · Design Tokens Guide (2026-02)](https://www.bootspring.com/blog/design-tokens-system-guide)
...
ParallelMe 完整规范见 `docs/design/VISUAL-SYSTEM.md` + `docs/design/TECH-ARCHITECTURE.md` § 7。

---

## 6 · ParallelMe 技术决策最终回放

参考 `docs/design/TECH-ARCHITECTURE.md` § 12：

```
✓ Next.js 14 + React 18 + TS 5
✓ Tailwind v3.4 + tailwind.config.ts theme.extend + CSS variables
✓ Dexie (IndexedDB)
