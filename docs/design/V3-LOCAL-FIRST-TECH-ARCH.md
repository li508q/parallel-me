...
> **Revision Notice (V3 开发前定稿)**：本文定义"本地优先 + 无状态服务端 + 用户可携带工作区"的底层架构。它与 [`V3-TECH-DIRECTION.md`](./V3-TECH-DIRECTION.md) 配合阅读：本文是"为什么 + 数据契约"，V3-TECH-DIRECTION 是"具体技术选型 + 实现细节"（Dexie / Vercel AI SDK 5 / Anthropic prompt caching / Tailwind v4 design tokens / SSE 状态机）。冲突时以 V3-TECH-DIRECTION 为准。
>
> 上层产品决策见 [`V3-IVY-FINAL-DIRECTION.md`](./V3-IVY-FINAL-DIRECTION.md)。
