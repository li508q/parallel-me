...
它不是用户可见产品面板，不应打断圆桌、不改变当前 stage，也不暴露内部标签。圆桌过程中它可以后台刷新；用户进入问询前，前端会再尝试同步刷新一次，尽量让最新圆桌历史进入最终问询。

观察账本虽然是后台对象，但仍使用严格结构化输出：当前版本为 `observation_ledger_v2`。模型必须返回可校验的观察、未回应问题和模块信号；如果结构不合格，系统先尝试修复，再进入重试。连续失败时只保留上一份可用账本，或让最终问询直接读取完整圆桌记录；系统不会补写模板观察，也不能生成用户可见的模板问题。
...
- `settlement_report` 必须携带完整 `AlignmentProfile`：被证伪幻想、核心价值主轴、已接纳代价、用户自我陈述和正反合都不能留空。
